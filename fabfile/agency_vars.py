...
from fabric.api import abort, cd, local
...
def get_static_url(d):
    ...
    with shell_env(**d):
        settings = parse_vars(local('./manage.py settings_vars STATIC_URL',capture=True))
...
def with_vars(f):
    """Decorator adds AGENCY_VARS to environment variables, if an env argument is passed the corresponding values from app_info.json will also be added to the env vars""" 
    @functools.wraps(f)
